...
public class HttpClientLauncher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (type == null) throw new IllegalArgumentException(usage());
    ...
    private static String usage() {
        return "Usage: java -cp ... " + HttpClientLauncher.class.getName() + " <URL> "
                + ClientType.pipeSeparatedString();
    }
    ...
    private static void runUsingJavaNet(String targetUrl, int connectTimeoutMillis, int readTimeoutMillis)
            throws IOException {
    ...
    private static void runUsingJdkHttpClient(String targetUrl, int connectTimeoutMillis, int readTimeoutMillis)
        ...
        log(
                "HttpClient timeouts: connection: %d, request: %d",
    ...
    private static void runUsingHttpClient3(String targetUrl, int connectTimeoutMillis, int readTimeoutMillis)
            throws IOException {
        ...
        if (connectTimeoutMillis != 0)
            client.getParams()
                    .setParameter(HttpConnectionParams.CONNECTION_TIMEOUT, Integer.valueOf(connectTimeoutMillis));
        if (readTimeoutMillis != 0)
        ...
        log(
                "Connection timeouts: connect: %d, so: %s",
                client.getHttpConnectionManager().getParams().getConnectionTimeout(),
                client.getHttpConnectionManager().getParams().getSoTimeout());
        log(
                "Client so timeout: %d (raw: %s)",
                client.getParams().getSoTimeout(), client.getParams().getParameter(HttpClientParams.SO_TIMEOUT));
        client.executeMethod(get);

        log(get.getStatusLine().toString());

        for (Header header : get.getResponseHeaders()) log(header.toExternalForm());
    }

    private static void runUsingHttpClient4(String targetUrl, int connectTimeoutMillis, int readTimeoutMillis)
            throws IOException {
        // disable retries, to make sure that we get equivalent behaviour with other implementations

        Builder config = RequestConfig.custom();
        if (connectTimeoutMillis != 0) config.setConnectTimeout(connectTimeoutMillis);
        if (readTimeoutMillis != 0) config.setSocketTimeout(readTimeoutMillis);
        ...
        try (CloseableHttpClient client = HttpClients.custom()
            ...
            try (CloseableHttpResponse response = client.execute(get)) {
                ...
                for (org.apache.http.Header header : response.getAllHeaders()) log(header.toString());
    ...
    private static void runUsingOkHttp(String targetUrl, int connectTimeoutSeconds, int readTimeoutSeconds)
            throws IOException {
        OkHttpClient.Builder clientBuilder = new OkHttpClient().newBuilder();
        if (connectTimeoutSeconds != 0) clientBuilder.connectTimeout(Duration.ofMillis(connectTimeoutSeconds));
        if (readTimeoutSeconds != 0) clientBuilder.readTimeout(Duration.ofMillis(readTimeoutSeconds));
